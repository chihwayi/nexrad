...
const config = {
  ...
  success: { icon: CheckCircle,   bg: 'bg-success/8 border-success/30', text: 'text-success', iconClass: 'text-success' },
  warning: { icon: AlertTriangle, bg: 'bg-warning/8 border-warning/30', text: 'text-warning', iconClass: 'text-warning' },
  error:   { icon: XCircle,       bg: 'bg-danger/8 border-danger/30',   text: 'text-danger',  iconClass: 'text-danger' },
}

export function AlertBanner({ variant, title, message, onDismiss, action, className }: AlertBannerProps) {
      ...
      <div className="flex-1 min-w-0">
        ...
        {action && (
          <button
            ...
            className={cn('text-xs font-semibold underline underline-offset-2 mt-1.5 hover:no-underline transition-all', text)}
          >
            {action.label}
          </button>
        )}
