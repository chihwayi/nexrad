  ...
  app.post<{ Params: { sessionId: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        // Attempt CoA disconnect via radclient using the branch's real RADIUS secret
        if (nasSecret) {
          execSync(
    ...
    },
    {
      ...
      cell: (r: BillingPlan) => (
        <button
          ...
          className={`${r.isActive ? 'badge-online' : 'badge-offline'} cursor-pointer hover:opacity-80`}
        >
